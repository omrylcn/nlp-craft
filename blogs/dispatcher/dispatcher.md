...
| Sorumluluk | Ne Yapar | Neden Tek Call'da? |
|------------|----------|-------------------|
...
| Context Katmanı | Ne İçerir | Neden Gerekli |
|-----------------|-----------|---------------|
| **Tool Registry** | Her tool'un ne yaptığı, parametreleri, dönüş şeması | "Tek tool yeter mi, birden fazla mı lazım?" değerlendirmesi |
| **Handler Tanımları** | Her handler ne yapar, hangi tool'ları birleştirir, hangi parametreleri bekler | "Mevcut bir handler bunu zaten çözüyor mu?" değerlendirmesi |
| **Workflow Tanımları** | Önceden tanımlanmış multi-step execution planları ve tool zincirleri | "Bu karmaşık istek için kanıtlanmış bir workflow var mı?" değerlendirmesi |
| **Conversation Context** | Daha önce ne konuşuldu, hangi veriler zaten alındı | "Önceki sonuçları yeniden kullanabilir miyim?" değerlendirmesi |
...
| Boyut | Katman 1 | Katman 2 | Katman 3 | Katman 4 |
|-------|----------|----------|----------|----------|
| Execution | Direkt tool call | Handler (deterministik) | Workflow (deterministik zincir) | Agent (sınırlı non-det.) |
| Tool sayısı | 1 | 1-2 | 2-4 | N (whitelist dahilinde) |
| LLM call (dispatcher hariç) | ❌ 0 | ❌ 0 | ⚠️ 0-1 (opsiyonel formatlama) | ✅ 1-5 (iteration'a bağlı) |
| Determinizm | ✅ Tam | ✅ Tam | ✅ Zincir sabit, opsiyonel LLM step | ⚠️ Sınırlı (bounded) |
| Test edilebilirlik | Kolay | Kolay | Kolay | Zor (ama loglanır) |
| Hallucination riski | Yok | Yok | Yok (opsiyonel LLM step'te minimal) | Düşük (guardrail'ler) |
| Policy gerekli mi? | Hayır | Hayır | Hayır | **Evet** |
| Trafik payı | ~%35-40 | ~%45-50 | ~%10 | ~%1-5 |

### Workflow vs. Agent vs. Constrained Agent: Kritik Ayrım

Bu, pattern'in kalbidir. Üç execution stratejisinin karşılaştırması:

| Boyut | Free Agent | Önceden Tanımlı Workflow | Constrained Agent (Katman 4) |
|-------|-----------|--------------------------|-------------------------------|
| Tool sırası | Runtime'da karar verilir (non-deterministic) | Kodda tanımlı (deterministic) | Runtime'da karar verilir ama whitelist dahilinde |
| Parametre akışı | Agent context'ten çıkarır (hata riski) | Açıkça kodlanmış (step_1.output → step_2.input) | Agent çıkarır ama sınırlı adımda |
| Maksimum iterasyon | Sınırsız | Sabit (workflow'daki step sayısı) | Sabit üst sınır (3-5) |
| Hallucination riski | Yüksek | Yok — tool çıktıları doğrudan kullanılır | Düşük — guardrail + doğrulama |
| Test edilebilirlik | Son derece zor | Standart unit testing | Zor ama tam loglama ile audit edilebilir |
| Yeni yetenek ekleme | Prompt'u güncelle ve umut et | Yeni workflow class'ı yaz | Otomatik — tool whitelist'te ise kullanır |
| Maliyet öngörülebilirliği | Değişken (agent döngüye girebilir) | Sabit (bilinen step sayısı) | Bounded (max iteration + timeout + budget) |
| Yönetim | Prompt engineering | Kod | **Policy (açılıp kapatılabilir)** |
...
| Durum | Aksiyon | Latency Etkisi |
|-------|---------|----------------|
| Agent 3s içinde ilk action yok | Hemen kes → Medium fallback | Max 3.5s (3s + fallback) |
| Agent çalışıyor ama 8s timeout | Kes → Medium fallback | Max 8.5s (timeout + fallback) |
...
| Bileşen | Hedef Token | Strateji |
|---------|------------|----------|
| Tool Registry | ~300-500 | Compact format: isim, tek satır açıklama, parametre listesi |
| Handler Tanımları | ~300-500 | İsim, kullandığı tool'lar, beklediği parametreler |
| Workflow Tanımları | ~200-400 | İsim, step listesi, ne zaman kullanılacağı |
| Routing Kuralları | ~200-300 | Öncelik sırası, fallback kuralları |
| **Toplam** | **~1-2K** | Context window'un %1-2'si |
...
| Metrik | Güvenli Bölge | Uyarı Bölgesi | Geçiş Düşünülmeli |
|--------|--------------|---------------|-------------------|
| Workflow sayısı | ≤10 | 10-20 | >20 |
| Fallback oranı | ≤%5 | %5-15 | >%15 |
| Misrouting oranı | ≤%3 | %3-10 | >%10 |
| Edge case oranı | ≤%5 | %5-10 | >%10 |
| Workflow ortalama ömrü | >3 ay | 1-3 ay | <1 ay |
...
| Metrik | Hedef | Uyarı Eşiği | Aksiyon |
|--------|-------|-------------|---------|
| Doğru katman seçim oranı | >%95 | <%90 | Dispatcher prompt review |
| Fallback oranı | <%5 | >%10 | Yeni handler/workflow ekle |
| Workflow coverage | >%90 | <%80 | Log analizi → yeni workflow |
| Ortalama latency (easy) | <200ms | >500ms | Tool performance check |
| Ortalama latency (medium) | <600ms | >1s | Handler optimization |
| Ortalama latency (hard) | <1.5s | >3s | Workflow step analizi |
| Katman 4 trafik payı | <%3 | >%5 | Agent log → workflow dönüşümü |
| Katman 4 başarı oranı | >%80 | <%60 | Guardrail/prompt review |
| Katman 4 günlük harcama | <$5 | >$10 | Policy budget'ı sık |
| Misrouting oranı | <%3 | >%5 | Routing kuralları güncelle |
...
| Metrik | Basit Router | Informed Dispatcher | Full Agent |
|--------|-------------|--------------------:|------------|
| Easy istek maliyeti | ~$0.001 (gereksiz LLM) | **~$0** (LLM bypass) | ~$0.005 |
| Medium istek maliyeti | ~$0.001 | ~$0.001 | ~$0.010 |
| Hard istek maliyeti | ❌ Desteklenmiyor | ~$0.005-0.010 | ~$0.015+ |
| Unknown istek maliyeti | ❌ Desteklenmiyor | ~$0.01-0.03 (Katman 4) | ~$0.015+ |
| Aylık toplam maliyet | ~$300 | **~$420** | $1,500-4,500 |
| Easy latency | ~500ms | **~100-200ms** | ~800ms |
| Medium latency | ~500ms | ~500ms | 1-3s |
| Hard latency | ❌ | ~800ms-1.5s (sabit) | 3-5s+ (değişken) |
| Unknown latency | ❌ | ~2-8s (bounded) | 3-5s+ (değişken) |
| Maliyet patlaması riski | Yok | **Yok** (policy budget) | Yüksek (agent döngüsü) |
| Hallucination riski | Düşük | **Düşük** | Yüksek |
| Test edilebilirlik | Kolay | **Kolay** (Katman 1-3) / Audit (Katman 4) | Çok zor |
| SLA garantisi | Yüksek | **Yüksek** (Katman 1-3) / Bounded (Katman 4) | Düşük |
